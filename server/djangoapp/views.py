...
def registration(request):
    # Get user data from request.body JSON dictionary
    data = json.loads(request.body)
    username = data['userName']
    password = data['password']
    ...
    # Check if a new user. If new - create a new user. If not new - not add
    # a new user, log error message "New user"
    try:
        User.objects.get(username=username)
        user_exist = True
    except Exception as err:
        logger.error(f"New user err: {err}")
...
def get_cars(request):
    ...
    if (count == 0):
        cnt_makes, cnt_models = initiate()
        print(f'Created {cnt_makes} records in CarMake DB, and {cnt_models} \
              records in CarModel DB')
    count = CarModel.objects.filter().count()
    print(f'{count} records in CarModel found')
